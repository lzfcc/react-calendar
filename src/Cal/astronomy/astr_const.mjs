...
export const modernConsts = (Jd) => {
  ...
  const perihelion =
    ...
    1.719175 * T1 +
    (1.63 / 3600) * T1 ** 2 +
    (0.012 / 3600) * T1 ** 3; //《古曆新探》頁322.近日點平黃經 ω // 近日點375年在大雪，1247年在冬至
  const Solar =
    365.242189623 - 0.000061522 * T - 6.09e-8 * T ** 2 + 2.6525e-7 * T ** 3; // VSOP87 曆表 Meeus J，Savoie D. The history of the tropical year[J]. Journal of the British Astronomical Association，1992，102( 1) : 42.
  ...
  const eccentricity = +Interpolate2(
    Julian2kyr,
    0.016702362,
    [0.00045902, -0.000123588, 0.000137078, -0.000134331]
  ).toFixed(9);
...
export const BindSolarChange = (year) => {
  ...
  const SolarWest = big(365.2422393296)
    .sub(big(6.16 * 1e-8).mul(year1))
    .toNumber();
  const SolarChangeWest = parseFloat(
    (
      sign1 *
      big(3.08 * 1e-8)
        .mul(year1 ** 2)
  ...
  const SolarShoushi = parseFloat(
    (365.2425 - 2 * 1e-6 * year2).toPrecision(10)
  );
  const SolarChangeShoushiRaw = parseFloat(
    (-Math.trunc(year2 / 100) / 10000).toPrecision(10)
  );
  const SolarChangeShoushi = parseFloat(
    (sign2 * -SolarChangeShoushiRaw * year2).toPrecision(10)
  );
  const SolarWannian = parseFloat(
    (365.2425 - 1.75 * 1e-6 * year2).toPrecision(10)
  );
  const SolarChangeWannian = parseFloat(
    (sign1 * 8.75 * 1e-7 * year2 ** 2).toPrecision(10)
  );
  const LunarWest = big(29.530587110428)
    .add(big(2.162 * 1e-9).mul(year1))
    .toNumber();
  const LunarCahngeWest =
    -sign1 *
    big(1.081 * 1e-9)
      .mul(year1 ** 2)
      .toNumber();
