...
const MoonLonTable = (NodeEclpGong, NowNodeDif, Name) => {
  ...
  const NodeDifQuarRev =
    NodeDifQuar < NodeQuar / 2 ? NodeDifQuar : NodeQuar - NodeDifQuar;
  // let Range = [];
  // if (Type === 6) { // 麟德沒有
  //   Range = [
...
export const eclp2WhiteDif = (NodeEclpGong, NowNodeDif, Name) => {
  ...
  const NodeDifHalf = NowNodeDif % NodeHalf;
  const NodeDifRev =
    NodeDifQuar < NodeOcta ? NodeDifQuar : NodeQuar - NodeDifQuar; // 到正半交的前後距離。紀元：置黃道宿積度，滿交象度去之，在半交象已下爲初限；已上者，以減交象度，餘爲入末限。
  // 判斷在正交前後還是半交前後
  ...
  const isYinSun =
    NodeEclpGong > SolarQuar && NodeEclpGong < SolarQuar3 ? 1 : -1; // 日行陰陽
  ...
  if (["Qintian", "Yingtian", "Qianyuan", "Yitian"].includes(Name)) {
    // sign1我重新整理了一遍思路
    if (NowNodeDif < NodeOcta || NowNodeDif >= NodeOcta * 7) {
      // 在正交前後各九限
      if (NodeEclpGong < SolarHalf) {
        sign1 = -1;
      }
    } else if (NowNodeDif >= NodeOcta && NowNodeDif < NodeOcta3) {
      // 在陽半交
      if (NodeEclpGong < SolarQuar || NodeEclpGong >= SolarQuar3) {
        // 秋分之宿後出黃道外
        sign1 = -1;
      }
    } else if (NowNodeDif >= NodeOcta3 || NowNodeDif < NodeOcta * 5) {
      // 在中交
      if (NodeEclpGong >= SolarHalf) {
        sign1 = -1;
      }
    ...
    if (NodeDifHalf >= NodeOcta && NodeDifHalf < NodeOcta * 3) {
      // 半交前後，距二分之宿限數
...
const moonJiudaoFormula = (NodeEclpGong, NewmEclpGong, Name, Y) => {
  ...
  const { Name: NodeMansName, MansDeg: NodeEclpMansDeg } = deg2Mans(
    NodeEclpDeg,
    EclpAccumList
  );
  for (let i = 0; i < EclpAccumList.length; i++) {
    const MansNodeDif_Eclp =
      (EclpAccumList[i] - NodeEclpDeg + Sidereal) % Sidereal; // 每宿距交
    WhiteAccumList[i] =
      EclpAccumList[i] + eclp2WhiteDif(NodeEclpGong, MansNodeDif_Eclp, Name);
  }
  ...
  const NodeWhiteMansDeg =
    NodeEclpMansDeg + eclp2WhiteDif(NodeEclpGong, NodeEclpMansDeg, Name); // 正交九道宿度
  const NodeWhiteDeg = mans2Deg(
    NodeMansName + NodeWhiteMansDeg,
    WhiteAccumList
  );
  const SolsNode_EclpDif = Solar - NodeEclpGong;
  const SolsNode_WhiteDif =
    SolsNode_EclpDif + eclp2WhiteDif(NodeEclpGong, SolsNode_EclpDif, Name);
  ...
  const NewmNode_WhiteDif =
    NewmNode_EclpDif + eclp2WhiteDif(NodeEclpGong, NewmNode_EclpDif, Name);
...
export const chooseNode = (
  ...
  if (Name === "Qintian") {
    ...
    const AcrNewmNodeAccum =
      AvgNewmNodeAccum + AutoTcorr(AvgNewmAnoAccum, AvgNewmSd, Name).Tcorr;
    ...
    NodeEclp =
      (AvgNodeEclpGong +
        AutoDifAccum(undefined, AvgNodeEclpGong, Name).SunDifAccum +
        Solar) %
      Solar;
  } else if (Name === "Yingtian" || Name === "Qianyuan") {
    const AcrNewmNodeAccum =
      AvgNewmNodeAccum + AutoTcorr(AvgNewmAnoAccum, undefined, Name).MoonTcorr;
    NodeEclp = (NewmEclpGong - MoonAvgVd * AcrNewmNodeAccum + Solar) % Solar;
  } else if (Name === "Yitian") {
    const AcrNewmAnoAccum =
      AvgNewmAnoAccum + AutoTcorr(AvgNewmAnoAccum, undefined, Name).MoonTcorr;
    const AcrNewmAnojour = anojour(AcrNewmAnoAccum, Name).Anojour;
    if (AvgNewmNodeAccum < Node / 2) {
      // 儀天用距離最近的正交
      const NodeAnojour = anojour(
        AvgNewmAnoAccum - AvgNewmNodeAccum,
        Name
      ).Anojour; // 「正交曆積度」。但是儀天沒有說怎麼求，按道理應該是這樣
      NodeEclp =
        (NewmEclpGong - (AcrNewmAnojour - NodeAnojour) + Solar) % Solar;
    } else {
      const Node1Anojour = anojour(
        AvgNewmAnoAccum + Node - AvgNewmNodeAccum,
        Name
      ).Anojour;
...
export const moonJiudao = (
  ...
  const { WhiteAccumList, NewmWhiteDeg, NewmWhiteGong, NodeWhiteGong } =
    moonJiudaoFormula(NodeEclp, NewmEclpGong, Name, Y);
  return {
    WhiteAccumList,
    NewmWhiteDeg,
    NewmWhiteGong,
...
export const MoonLatTable = (NodeAccum, Name) => {
  ...
  if (Type <= 4) Portion = 12;
    ...
    Portion = 50 / 3; // 交率交數61、777，去交度乘數十一，除數千一百六十五
  ...
  if (Type <= 4) {
    Lat =
      ...
      ((NodeAccumHalf - NodeAccumHalfInt) * MoonLatDifList[NodeAccumHalfInt]) /
        Portion;
...
export const equa2WhiteDif = (Dingxian, WhEq_Dif) => {
  ...
  const XQuarRev = XQuar < SolarOcta ? XQuar : SolarQuar - XQuar; // 大統「月道與赤道正交後積度 并入初末限」赤道二十八宿到白赤交點半交的距離，半「氣象限（Solar25）」以內
...
export const moonShoushi = (
  ...
  const Node1EclpHalfRev =
    Node1EclpHalf < SolarQuar ? Node1EclpHalf : SolarHalf - Node1EclpHalf; // 求正交在二至後初末限：置冬至距正交積度及分，在半歲周已下，為冬至後；已上，去之，爲夏至後。其二至後，在象限已下，爲初限，已上，減去半歲周，爲末限
  ...
  const Newm_WhEq_WhiteDif =
    Newm_WhEq_EquaDif + equa2WhiteDif(Dingxian, Newm_WhEq_EquaDif);
  /// 九道宿鈐
  const WhiteAccumList = [];
  let NewmWhiteDeg = 0,
    NewmWhiteGong = 0,
  ...
  if (Y !== undefined) {
    ...
    for (let i = 0; i < EquaAccumList.length; i++) {
      const Mans_WhEq_Dif =
        (EquaAccumList[i] - WhEq_EquaDeg + Sidereal) % Sidereal; // 「正交後積度」
      WhiteAccumList[i] =
        Mans_WhEq_Dif + equa2WhiteDif(Dingxian, Mans_WhEq_Dif);
    }
    const adj = WhiteAccumList[0] - Sidereal;
    for (let i = 0; i < WhiteAccumList.length; i++) {
      WhiteAccumList[i] = (WhiteAccumList[i] - adj + Sidereal) % Sidereal;
    ...
    WhiteAccumList[28] = Sidereal;
    const { Name: WhEq_MansName, MansDeg: WhEq_EquaMansDeg } = deg2Mans(
      WhEq_EquaDeg,
      EquaAccumList
    ); // 白赤正交宿度
    const WhEq_WhiteMansDeg =
      WhEq_EquaMansDeg + equa2WhiteDif(Dingxian, WhEq_EquaMansDeg);
    const WhEq_WhiteDeg = mans2Deg(
      WhEq_MansName + WhEq_WhiteMansDeg,
      WhiteAccumList
    ); // 正交宿度赤轉白，授時曆沒有，我覺得按道理應該有
    NewmWhiteDeg = (WhEq_WhiteDeg + Newm_WhEq_WhiteDif) % Sidereal; // 定朔白道宿積度
    /// 冬至月道宿度
    const Sols_WhEq_EquaDif = Solar - WhEqGong;
    const Sols_WhEq_WhiteDif =
      Sols_WhEq_EquaDif + equa2WhiteDif(Dingxian, Sols_WhEq_EquaDif);
  ...
  if (NowNewm_WhiteDif) {
    ...
    const HM =
      NowWhEq_WhiteDifHalf < SiderealQuar
        ? NowWhEq_WhiteDifHalf
        : SiderealHalf - NowWhEq_WhiteDifHalf; // 每日月離赤道交後初末限
    const MK = SiderealQuar - HM; // 白道積
    const UK = RoundL2H(MK); // 矢「每日積差」
    // const Theta1 = WhEqObliqMax / SiderealSext; // 定差。注意，有兩個「定差」。下一步：「餘以定差乘之」
    EquaLat =
      ((NowWhEq_WhiteDif < SiderealHalf ? 1 : -1) *
        (WhEqObliqMax * (SiderealSext - UK))) /
      SiderealSext; // 與《數理》p383核驗無誤。每日月離赤道內外度。內減外加象限，爲每日月離白道去極度
  }
  return {
    WhiteAccumList,
    EquaLat,
