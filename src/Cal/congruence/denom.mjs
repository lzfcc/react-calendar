...
import { GcdLcm } from "../congruence/gcdlcm.mjs";
import { CongruenceModulo } from "../congruence/qiuyi.mjs";

export const DecomposePrimeFactor = (a, b, BigNumer, BigDenom) => {
  // 「累強弱之數」：日法朔餘都要有。有些宋曆算不出來，不知道是不是我寫的有問題。
  ...
  const SmallPrint =
    ...
    SmallDenom +
    " ≈ " +
    parseFloat((SmallNumer / SmallDenom).toPrecision(12));
  a = +a;
  b = +b;
  BigNumer = +BigNumer;
  BigDenom = +BigDenom;
  ...
  if (Math.trunc(a) !== a) {
    //這行是處理小數。輸入的一般都是整數，萬一有小數，一位*10，兩位*100
    a *= 10;
    b *= 10;
    if (Math.trunc(a) !== a) {
      a *= 10;
  ...
  const Result2a =
    a1_2 + " = " + BigNumer + "×" + M1_2 + " + " + SmallNumer + "×" + M2_2;
  const Result2b =
    b1_2 + " = " + BigDenom + "×" + M1_2 + " + " + SmallDenom + "×" + M2_2;
  ...
  const Result3a =
    a1_3 + " = " + BigNumer + "×" + M1_3 + " + " + SmallNumer + "×" + M2_3;
  const Result3b =
    b1_3 + " = " + BigDenom + "×" + M1_3 + " + " + SmallDenom + "×" + M2_3;
  ...
  const Deci = (M1_1, M2_1) =>
    (BigNumer * M1_1 + SmallNumer * M2_1) /
    (BigDenom * M1_1 + SmallDenom * M2_1);
  while (b1_1 < b) {
    if (Deci(M1_1, M2_1) < parseFloat((a / b).toPrecision(14))) {
      // 棄左行
      L1 = M1_1;
      L2 = M2_1;
    } else {
