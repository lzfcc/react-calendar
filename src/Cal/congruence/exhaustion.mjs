import { IndetermEqua } from "../congruence/origin.mjs";
    ...
    if (c > lower && c < upper) {
      if (Anoma > x - Range && Anoma < x + Range) {
        result +=
          Int +
          "+" +
          (FracInt + i * step).toFixed(4) +
          "/" +
          Denom +
          " ≈ " +
          (Int + (FracInt + i * step).toFixed(4) / Denom).toFixed(8) +
          " 入日" +
          (SolsAccum % (Int + (FracInt + i * step) / Denom)).toFixed(4) +
          "\n";
      }
    }
  }
