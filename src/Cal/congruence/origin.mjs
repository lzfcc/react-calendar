...
export const Sunzi = (InputRaw) => {
  ...
  while (BianNum < Yuan.length) {
    for (let k = Yuan.length - 1 - BianNum; k >= 1; k--) {
      ...
      while (k - 1 - LoopNum >= 0) {
        let gcd = GcdLcm(DingRaw[k], DingRaw[k - 1 - LoopNum]).gcd;
        DingRaw[k - 1 - LoopNum] = big
          .div(DingRaw[k - 1 - LoopNum], gcd)
          .toNumber();
        gcd = GcdLcm(DingRaw[k], DingRaw[k - 1 - LoopNum]).gcd;
        while (big(gcd).gt(1)) {
          // if gcd > 1
          DingRaw[k - 1 - LoopNum] = big
            .mul(DingRaw[k - 1 - LoopNum], gcd)
            .toNumber();
...
///////// 解二元一次不定方程
export const IndetermEqua1 = (a, b, z) => {
  // 二元一次不定方程
  a = Math.trunc(a);
  b = Math.trunc(b);
  z = Math.trunc(z);
...
// 秦九韶調日法
export const IndetermEqua = (a, b, z) => {
  // a彊母b弱母
  a = Math.trunc(a);
  b = Math.trunc(b);
  z = Math.trunc(z);
...
export const ZhangModulo = (SolarFrac, SolarDenom, LunarFrac, Denom) => {
  ...
  const ZhangMon =
    (ZhangRange * SolarNumerRaw * Denom) / SolarDenom / LunarNumerRaw;
  const ZhangLeap = ZhangMon - 12 * ZhangRange;
  const BuRangeRaw = FracLcm1([
    SolarNumerRaw,
    SolarDenom,
    LunarNumerRaw,
    Denom,
  ...
  const BuRange = big
    .mul(big(BuRangeRaw).div(SolarNumerRaw), SolarDenom)
    .toString();
  const YuanRange = big
    .mul(big(YuanRangeRaw).div(SolarNumerRaw), SolarDenom)
    .toString();
...
export const OriginModulo2 = (
  ...
  const Result =
    Sunzi(OriginComm + "," + ScComm + "," + LeapComm + "," + LunarComm).x /
    SolarComm;
...
export const OriginModulo = (
  ...
  const LeapContrac =
    LunarNumer + FirstConst - ((YearLeap * YuanYear) % LunarNumer); // 188578閏縮。(YearLeap * YuanYear % LunarNumer) 爲入閏
