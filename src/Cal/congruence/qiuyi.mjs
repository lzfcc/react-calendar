...
//     }
// }
export const CongruenceModulo = (aRaw, bRaw) => {
  // 大衍求一術：奇數 * x ≡ 1 (mod 592)，求 x。 https://zhuanlan.zhihu.com/p/272302805
  //   L1=1 | a奇數
  //   -----+-----
  //   L2=0 | b定母
  let a = new big(aRaw);
  ...
  while (i % 2 === 0 ? big(a).gt(1) : big(b).gt(1)) {
    if (i % 2 === 0) {
      L2 = big(L1)
        .mul(big.floor(big.max(a, b).div(big.min(a, b))))
        .add(L2);
  ...
  if (big.mul(aRaw, Result).mod(bRaw).eq(1)) {
  } else {
    throw new Error("[大衍求一] 兩數不互質！");
  }
