...
export default (YearStart, YearEnd, Longitude, Latitude, h, MansSystem) => {
  ...
  const Main = (Y) => {
    const { LeapNumTerm, NewmUT1JdPrint: NewmUT1Jd } = CalNewm(
      Y,
      Y,
      Longitude
    )[0];
    ...
    for (let i = 1; i <= 12 + (LeapNumTerm > 0 ? 1 : 0); i++) {
      ...
      (Sc[i] = []),
        ...
        (Duskstar[i] = []),
        (Pos[i] = []);
      const { EclpAccumList, EquaAccumList, CecAccumList } = mansModernList(
        NewmUT1Jd[i - 1] + 15,
        MansSystem
      ); // 取月中的宿積度表，減少計算次數
      for (
        let k = 1;
        k <=
        Math.trunc(NewmUT1Jd[i] - 0.5) - Math.trunc(NewmUT1Jd[i - 1] - 0.5);
        k++
      ) {
        DayAccum++;
        const DeltaT = deltaT(NewmUT1Jd[i - 1] + 15); // 本月DeltaT
        const LocalMidnUT1Jd = Math.round(NewmUT1Jd[i - 1] + k - 1) - 0.5; // 儒略日半夜是0.5，所以要四捨五入再減.5
        Jd[i][k] = LocalMidnUT1Jd + 0.5;
        ...
        const {
          ...
          HoriLon,
          HoriLat
        } = bindTopo_vsop(MidnTTJd, Longitude, Latitude, h);
        const SunEclpLatNoon = calPos_vsop("Sun", MidnTTJd + 0.5).EquaLat;
        Pos[i][k] = [];
        for (let j = 0; j < 7; j++) {
          // 七政赤道、極黃、黃道、入宿度
          ...
          Pos[i][k].push(
            {
              [PlanetList[j] + "HoriLon"]: (HoriLon[j] * R2D).toFixed(4),
              [PlanetList[j] + "HoriLat"]: lat2NS(HoriLat[j] * R2D)
            },
            {
              [PlanetList[j] + "EquaLon"]: (EquaLon[j] * R2D).toFixed(4),
              [PlanetList[j] + "EquaLat"]: lat2NS(EquaLat[j] * R2D)
            },
            {
              [PlanetList[j] + "CecLon"]: CecLon[j].toFixed(4),
              [PlanetList[j] + "CecLat"]: lat2NS(CecLat[j])
            },
            {
              [PlanetList[j] + "EclpLon"]: (EclpLon[j] * R2D).toFixed(4),
              [PlanetList[j] + "EclpLat"]: lat2NS(EclpLat[j] * R2D)
            },
            {
              [PlanetList[j] + "Equa"]: EquaMans,
              [PlanetList[j] + "Cec"]: CecMans,
              [PlanetList[j] + "Eclp"]: EclpMans
            }
          );
        }
        ...
        Morningstar[i][k] = [
          {
            Morningstar: midstarModern(
              MidnTTJd + Morning,
              Longitude,
              EquaAccumList
            ),
            Morning: deci2hms(Morning).hm,
            SunRise: deci2hms(Rise).hm
          },
          {
            SunSet: deci2hms(Set).hm,
            Dusk: deci2hms(Dusk).hm,
            Duskstar: midstarModern(MidnTTJd + Dusk, Longitude, EquaAccumList)
          }
        ];
        ///////////具注曆////////////
