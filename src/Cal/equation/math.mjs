...
export const isSame = (arr1, arr2) => {
  // 判斷元數定母是否想等 @lzfcc 时间复杂度2*O(nlog(n)) + O(n) = O(nlog(n))
...
const Deci2Int_1 = function () {
  ...
  for (let j = 0; j < Raw.length; j++) {
    ...
    while (i < 11) {
      if (
        parseFloat((Int[j] * 10 ** i).toPrecision(12)) ===
        Math.floor(Int[j] * 10 ** i)
      ) {
...
export const DeciFrac2IntFrac = (Deci) => {
  // @lzfcc 把有小數點的分數轉換為整數分數
...
export const SliceNum = (Input, num = 3) => {
  // a：小數點前，b：小數點後，c：并之
