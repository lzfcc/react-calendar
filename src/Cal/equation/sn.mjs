...
export const Sn1 = (a, b, n) => {
  // 上有寬a個、長b個，下有寬c個、長d個，n層，每層長寬各多1個。芻童就是長方稜臺
...
export const Interpolate1 = (n, initial, highPrecisionMode = false) => {
  if (highPrecisionMode) {
    return Interpolate1_big(n, initial);
  }
  let S = 0,
    S4 = 0;
  let delta = [];
...
// 跟下面的區別是沒用Deci.js
export const Interpolate2 = (n, f0, delta, highPrecisionMode = false) => {
  if (highPrecisionMode) {
    return Interpolate2_big(n, f0, delta);
  }
...
const Interpolate2_big = (n, f0, delta) => {
  // delta是string。第一個數n是0，上面的函數第一個是1
...
export const Interpolate3 = (n, points, highPrecisionMode = false) => {
  if (highPrecisionMode) {
    return Interpolate3_big(n, points);
  }
  const x = [],
    y = [];
...
export const Interpolate3_big = (n, points) => {
  const x = [],
    y = [];
  for (let i = 0; i < points.length; i++) {
    x[i] = points[i][0];
