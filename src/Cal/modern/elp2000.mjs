...
export const elp2000 = (Jd) => {
  const argumentPolynomialCoefficients = [
    ...
    [785939.8782, 1732559343.3328, -6.87, 0.006604, -0.00003169],
    ...
    [361679.188, 129597742.3016, -0.0202, 0.000009, 0.00000015],
  ...
  const corrections = {
    deltanu: (0.55604 * S2R) / argumentPolynomialCoefficients[0][1],
    deltaE: 0.01789 * S2R,
    deltaGamma: -0.08066 * S2R,
    deltanp: (-0.06424 * S2R) / argumentPolynomialCoefficients[0][1],
    deltaep: -0.12879 * S2R
  };
  ...
  function evaluatePlanetaryArguments(t) {
    ...
    for (let i = 0; i < planetaryPolynomialCoefficients.length; i++) {
      planetaryArguments.push(
        planetaryPolynomialCoefficients[i][0] +
  ...
  function computeMainFigureCos(delArgs, data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let A =
        data[i].A -
        (2.0 / 3.0) * data[i].A * corrections.deltanu +
        (data[i].B1 + dtasm * data[i].B5) *
          (corrections.deltanp - am * corrections.deltanu) +
        data[i].B2 * corrections.deltaGamma +
        data[i].B3 * corrections.deltaE +
  ...
  const elp2000Main = (Jd) => {
    ...
    const zeta =
      argumentPolynomialCoefficients[0][0] +
    ...
    longitude +=
      computePlanetary1(planetaryArgs, delArgsLin, Elp2000List.ELP13) * T;
    latitude +=
      computePlanetary1(planetaryArgs, delArgsLin, Elp2000List.ELP14) * T;
    distance +=
      computePlanetary1(planetaryArgs, delArgsLin, Elp2000List.ELP15) * T;
    // Planetary perturbations. Table 2.
    longitude += computePlanetary2(
      planetaryArgs,
      delArgsLin,
      Elp2000List.ELP16
    );
    ...
    longitude +=
      computePlanetary2(planetaryArgs, delArgsLin, Elp2000List.ELP19) * T;
    latitude +=
      computePlanetary2(planetaryArgs, delArgsLin, Elp2000List.ELP20) * T;
    distance +=
      computePlanetary2(planetaryArgs, delArgsLin, Elp2000List.ELP21) * T;
    // Tidal effects.
    ...
    longitude +=
      computeNonPlanetary(0.0, delArgsLin, Elp2000List.ELP34) * T * T;
    ...
    const P =
      ...
      0.47020439e-6 * T * T -
      0.5417367e-9 * T * T * T -
      0.2507948e-11 * T * T * T * T +
      0.463486e-14 * T * T * T * T * T;
    const Q =
      -0.113469002e-3 * T +
      0.12372674e-6 * T * T +
      0.1265417e-8 * T * T * T -
      0.1371808e-11 * T * T * T * T -
    ...
    const xJ2000 =
      (1 - 2 * P * P) * xMod + 2 * P * Q * yMod + 2 * P * sqrtTerm * zMod;
    const yJ2000 =
      2 * P * Q * xMod + (1 - 2 * Q * Q) * yMod - 2 * Q * sqrtTerm * zMod;
    const zJ2000 =
      -2 * P * sqrtTerm * xMod +
      2 * Q * sqrtTerm * yMod +
      (1 - 2 * P * P - 2 * Q * Q) * zMod;
