...
const aa = (T) => {
  ...
  let F = [
    134.96340251 * 3600 +
      ...
      31.8792 * T ** 2 +
      0.051635 * T ** 3 -
      0.0002447 * T ** 4, // F1≡l月球平近點角
    357.52910918 * 3600 +
      129596581.0481 * T -
      0.5532 * T ** 2 +
      0.000136 * T ** 3 -
      0.00001149 * T ** 4, // F2≡l'太陽平近點角
    93.27209062 * 3600 +
      1739527262.8478 * T -
      12.7512 * T ** 2 -
      0.001037 * T ** 3 +
      0.00000417 * T ** 4, // F3≡F- omega 月球平黃經-月軌升交平黃經
    297.85019547 * 3600 +
      1602961601.209 * T -
      6.3706 * T ** 2 +
      0.006593 * T ** 3 -
      0.00003169 * T ** 4, // F4≡D 月日平距角
    125.04455501 * 3600 -
      6962890.5431 * T +
      7.4722 * T ** 2 +
      0.007702 * T ** 3 -
      0.00005939 * T ** 4 // F5≡omega月軌升交平黃經
  ];
...
const aa1 = (T) => {
  ...
  const F = [
    ...
    2.1824392 - 33.757045 * T, // Mean longitude of the ascending node of the Moon. aom
    4.402608842 + 2608.7903141574 * T, // F6≡LMercury
    3.176146697 + 1021.3285546211 * T, // F7≡LVenus
    1.753470314 + 628.3075849991 * T, // F8≡LEarth
    6.203480913 + 334.06124267 * T, // F9≡LMars
    0.599546497 + 52.9690962641 * T, // F10≡LJupiter
    0.874016757 + 21.329910496 * T, // F11≡LSaturn
    5.481293872 + 7.4781598567 * T, // F12≡LUranus
    5.311886287 + 3.8133035638 * T, // F13≡LNeptune
...
export const nutaMx = (T) => {
  ...
  const N = chain(rr1(-Obliq))
    .multiply(rr3(-NutaEclp))
    .multiply(rr1(ObliqAvg))
    .done();
  return { N, Obliq };
};
