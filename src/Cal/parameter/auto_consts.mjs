...
export const AutoQuar = (Name, Type) => {
  // 盈縮限
  let QuarA = 0;
  let QuarB = 0;
  if (Name === "Yitian") {
...
export const AutoMoonAvgV = (Name) => {
  // 陳美東《月離表初探》
  ...
  if (Name === "Daye") {
    ...
    // 7290/545.3=13.3687878232，按照公式=13.3687753161
    V = 7290 / 545.3; // 紀元
  } else if (Type === 10) {
    ...
    V = parseFloat(
      ((Sidereal || Solar) / (Lunar || LunarRaw) + 1).toPrecision(14)
    );
  }
  return V;
};
...
export const AutoLightRange = (Name) => {
  // 昏明
...
export const AutoRangeEcli = (Name, Type) => {
  // 日出入前後多少不算日月食
  let RangeSunEcli = 0,
    RangeMoonEcli = 0;
  if (Type === 11) {
...
export const AutoMoonTcorrDif = (AnoAccum, Name) => {
  // 唐宋月離損益率
  ...
  const AnomaQuar3 = Anoma * 0.75; // 20.6659
  let MoonTcorrDif = 0;
  let TheDenom = 1;
  if (Name === "Yitian") {
