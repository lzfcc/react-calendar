...
const ClockTmp = (Deci, Mode) => {
  // 我假設：每日96刻，子初夜半，每刻100分
  let Portion1 = 0;
  let Portion2 = 0;
  if (Mode === 96) {
    Portion1 = 0.96;
...
const ClockTang = (Deci) => {
  // 唐、宋皇祐之前。1/3刻放在時辰最後，可能是初或正兩種情況
...
const ClockSong = (Deci) => {
  // 皇祐之後、元、明。四刻是1/6。1刻60分，1分=14.4s
  const KeRaw = (Deci + 100 / 24 + 1e-10) % 100; // 夜半子半
  let ClockOrder = Math.trunc(KeRaw / (100 / 12));
  ...
  const MinOrder = Math.trunc(
    (KeRaw - (ClockOrder * (100 / 12) + HalfOrder * (4 + 1 / 6) + QuarOrder)) *
      60
  );
  return (
    BranchList[ClockOrder + 1] +
    HalfList[HalfOrder] +
    "" +
    QuarList[QuarOrder] +
    "刻" +
    nzh.encodeS(MinOrder) +
    "分"
  );
};
...
const ClockQing = (DeciRaw) => {
  // 清代96刻
  const { ClockOrder, HalfOrder, QuarOrder, MinOrder, SecOrder } =
    clockQingMain(DeciRaw);
  return (
    BranchList[ClockOrder + 1] +
    HalfList[HalfOrder % 2] +
    "" +
    QuarList[QuarOrder] +
    "刻" +
    (MinOrder === 0 ? "" : nzh.encodeS(MinOrder) + "分") +
    (SecOrder === 0 ? "" : nzh.encodeS(SecOrder) + "秒")
  );
};
...
export const BindClock1 = (Deci) => {
  ...
  Deci *= 100 + 1e-12;
  Print = Print.concat(
    ["Yuanjia", "Wuyin", "Easthan", "Huangji", "Dayan", "Mingtian"].map(
      (title) => {
        return {
...
export const Deci2Stage = (Deci) => {
  ...
  const FourB = Math.trunc((Frac + 0.125) * 4);
  if (Twelve === 11) {
    Order12++;
  }
  if (FourB === 4) {
